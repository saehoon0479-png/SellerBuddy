...
type DeepStrategyUiResult = {
  competitors?: Array<{
    name: string
    positioning: string
    tone: 'Functional' | 'Aesthetic' | 'Emotional' | 'Corporate'
...
const parseKeywordLayerItems = (value: string): string[] =>
  value
    .split(/[|,/]/)
    .map((item) => item.trim())
    .filter(Boolean)

const parseKeywordLayers = (value: string): DeepStrategyUiResult['keyword_layers'] => {
  const layers = String(value || '')
    .split('>')
    .map((part) => part.trim())
    .filter(Boolean)
  return {
    primary: parseKeywordLayerItems(layers[0] || ''),
    secondary: parseKeywordLayerItems(layers[1] || ''),
    gift_intent: parseKeywordLayerItems(layers[2] || ''),
    seasonal: [],
  }
}
...
const normalizeDeepStrategyResult = (raw: any): DeepStrategyUiResult | null => {
  ...
  // Keep legacy deep shape as-is when present.
  if (raw.keyword_layers && typeof raw.keyword_layers === 'object') {
    return raw as DeepStrategyUiResult
  }

  const hook = String(raw.HOOK ?? raw.hook ?? '').trim()
  const positioning = String(raw.POSITIONING ?? raw.positioning ?? '').trim()
  const keywordLayersRaw = String(raw.KEYWORD_LAYERS ?? raw.keyword_layers ?? '').trim()
  const creatorPrompt = String(raw.CREATOR_PROMPT ?? raw.creator_prompt ?? '').trim()

  if (!hook && !positioning && !keywordLayersRaw && !creatorPrompt) {
    return null
  }
  ...
  return {
    competitors: [],
    market_saturation: [],
    underserved_angle: hook || positioning,
    keyword_layers: parseKeywordLayers(keywordLayersRaw),
  ...
  const handleCompetitorImageSelect = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (totalAfterAdd > imageLimit) {
      if (effectivePlan === 'free') {
        setUpgradeModalMessage({
          title: 'Image limit reached',
          body: 'Free plan allows 1 image per competitor. Upgrade to Pro to upload up to 3 images per competitor.',
        })
        setUpgradeModalOpen(true)
      } else {
        setError(`Maximum ${imageLimit} image(s) allowed per competitor`)
      }
  ...
  const handleGenerate = async () => {
   ...
   const isDeepMode = activeMode === 'deep'
    ...
    try {
      ...
      if (isDeepMode) {
        const deepRaw = json?.data ?? json?.result?.deep_strategy ?? json?.deep_strategy ?? json?.result
        const deep = normalizeDeepStrategyResult(deepRaw)
        if (!deep) {
          setDeepError({
            code: 'MISSING_DEEP_RESULT',
            message: 'Deep Strategy result missing in response',
          })
          setDeepStrategyResult(null)
        } else {
    ...
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        ...
        <div className={styles.card}>
          <h2 className={styles.cardTitle}>Build your listing</h2>
              ...
              {competitors.map((competitor, index) => (
                <div key={index} className={styles.competitorCard}>
                  ...
                  <div className={styles.competitorImageSection}>
                    ...
                    <p className={styles.helper}>
                      {!mounted || plan === 'loading' ? 'Images: up to 3 (plan-based)' : plan === 'pro' ? 'Pro: up to 3 images' : 'Free: 1 image'}
                    </p>
                    <div className={styles.imageUploadZone}>
                      <input
                   ref={(el) => {
                    competitorFileInputRefs.current[index] = el
                        ...
                        className={styles.fileInput}
                      />
                      <label htmlFor={`competitor-images-${index}`} className={styles.uploadButton}>
                        Upload images
                      </label>
                    </div>
          ...
          >
            ...
            {deepStrategyResult && !deepError && (
              <div className={styles.card}>
                ...
                <div className={styles.deepStrategyReport}>
                  ...
                  {deepStrategyResult.competitors && deepStrategyResult.competitors.length > 0 && (
                    <div className={styles.strategySection}>
                      <h3 className={styles.strategySectionTitle}>Competitors</h3>
                      {deepStrategyResult.competitors.map((comp, idx) => (
                        <div key={idx} className={styles.resultSection}>
                          <div className={styles.resultValue}>
                            <strong>{comp.name}</strong> | {comp.tone}
                          </div>
                          <div className={styles.resultValue}>Positioning: {comp.positioning}</div>
                          <div className={styles.resultValue}>Emphasis: {comp.what_they_emphasize}</div>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {deepStrategyResult.market_saturation && (
                    <div className={styles.strategySection}>
                      ...
                      <div className={styles.keywordLayerTags}>
                        {deepStrategyResult.market_saturation.filter(Boolean).map((sat, idx) => (
                          <span key={idx} className={styles.keywordTag}>{sat}</span>
                        ))}
                      </div>
                    </div>
                  )}

                  {deepStrategyResult.keyword_layers && (
                    <div className={styles.strategySection}>
                      <h3 className={styles.strategySectionTitle}>Keyword Layers</h3>
                      <div className={styles.keywordLayers}>
                        <div className={styles.keywordLayerRow}>
                          <span className={styles.keywordLayerLabel}>Primary:</span>
                          <div className={styles.keywordLayerTags}>
                            {deepStrategyResult.keyword_layers.primary.filter(Boolean).map((kw, idx) => (
                              <span key={idx} className={styles.keywordTag}>{kw}</span>
                            ))}
                          </div>
                        </div>
                        <div className={styles.keywordLayerRow}>
                          <span className={styles.keywordLayerLabel}>Secondary:</span>
                          <div className={styles.keywordLayerTags}>
                            {deepStrategyResult.keyword_layers.secondary.filter(Boolean).map((kw, idx) => (
                              <span key={idx} className={styles.keywordTag}>{kw}</span>
                            ))}
                          </div>
                        </div>
                        <div className={styles.keywordLayerRow}>
                          <span className={styles.keywordLayerLabel}>Gift Intent:</span>
                          <div className={styles.keywordLayerTags}>
                            {deepStrategyResult.keyword_layers.gift_intent.filter(Boolean).map((kw, idx) => (
                              <span key={idx} className={styles.keywordTag}>{kw}</span>
                            ))}
                          </div>
                        </div>
                        <div className={styles.keywordLayerRow}>
                          <span className={styles.keywordLayerLabel}>Seasonal:</span>
                          <div className={styles.keywordLayerTags}>
                            {(deepStrategyResult.keyword_layers.seasonal || []).filter(Boolean).map((kw, idx) => (
                              <span key={idx} className={styles.keywordTag}>{kw}</span>
                            ))}
                          </div>
                        </div>
                  ...
                  {deepStrategyResult.creator_prompt && (
                    <div className={styles.strategySection}>
                      <h3 className={styles.strategySectionTitle}>Creator Prompt</h3>
            ...
            {deepStrategyResult && deepStrategyResult.creator_prompt && (
              <div className={styles.card}>
                <h2 className={styles.cardTitle}>Creator Prompt (for Listing Creator)</h2>
